...
def _write_readme(path: Path, summary: dict[str, object], normalized_root: Path) -> None:
    ...
    text = f"""# Spectral Library Package

This package is the packaged spectral-library export built from:
...
The exported tables follow the existing `siac_*` naming convention for
compatibility with downstream tooling and prepared-runtime builders.
...
def build_library_package(
    ...
    if missing:
        ...
        raise FileNotFoundError(f"Missing library packaging inputs: {missing_text}")
