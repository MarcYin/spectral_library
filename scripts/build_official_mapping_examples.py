...
EXAMPLE_K = 10
EXAMPLE_NEIGHBOR_ESTIMATOR = "simplex_mixture"
BENCHMARK_NEIGHBOR_ESTIMATORS = ("mean", "distance_weighted_mean", "simplex_mixture")
OVERLAY_TOP_NEIGHBORS = 5
DOC_NEIGHBOR_TABLE_TOP_N = 3
...
def _holdout_batch_neighbor_review_path() -> Path:
    return RESULTS_ROOT / "selected" / (
        f"{SENSOR_FILE_LABELS[HOLDOUT_BATCH_SOURCE_SENSOR]}_to_{SENSOR_FILE_LABELS[HOLDOUT_BATCH_TARGET_SENSOR]}_holdout_neighbor_review.csv"
    )


def _estimator_benchmark_csv_path() -> Path:
    return RESULTS_ROOT / "metrics" / "neighbor_estimator_holdout_comparison.csv"


def _estimator_benchmark_json_path() -> Path:
    return RESULTS_ROOT / "metrics" / "neighbor_estimator_holdout_comparison.json"
...
def _write_selected_cli_outputs(prepared_root: Path) -> None:
    ...
    _run_cli(
        ...
        str(_holdout_batch_neighbor_review_path()),
    )
...
def _write_pairwise_metrics(
    ...
    *,
    ...
    holdout_sample_ids = tuple(target.sample_id for target in HELD_OUT_TARGETS)
    holdout_exclude_row_ids = tuple(target.row_id for target in HELD_OUT_TARGETS)
    ...
    for source_sensor in SENSOR_BY_ID:
        for target_sensor in SENSOR_BY_ID:
            ...
            batch_result = mapper.map_reflectance_batch(
                ...
                exclude_row_ids_per_sample=holdout_exclude_row_ids,
            )
            sample_mae: list[float] = []
            sample_rmse: list[float] = []
            for sample_id, mapping_result in zip(batch_result.sample_ids, batch_result.results):
...
def _write_neighbor_estimator_holdout_comparison(
    prepared_root: Path,
    truth_rows_by_sensor: dict[str, dict[str, dict[str, float]]],
    ...
    for neighbor_estimator in BENCHMARK_NEIGHBOR_ESTIMATORS:
        pairwise_metrics = _write_pairwise_metrics(
            prepared_root,
            truth_rows_by_sensor,
            neighbor_estimator=neighbor_estimator,
            persist_outputs=False,
        )
        pairwise_values = list(pairwise_metrics.values())
        selected_pair_metrics = pairwise_metrics[(HOLDOUT_BATCH_SOURCE_SENSOR, HOLDOUT_BATCH_TARGET_SENSOR)]
        comparison_rows.append(
            {
                "neighbor_estimator": neighbor_estimator,
                "aggregate_mean_abs_error": float(np.mean([payload["mean_abs_error"] for payload in pairwise_values])),
                "aggregate_rmse": float(np.mean([payload["rmse"] for payload in pairwise_values])),
                "selected_pair_source_sensor": HOLDOUT_BATCH_SOURCE_SENSOR,
                "selected_pair_target_sensor": HOLDOUT_BATCH_TARGET_SENSOR,
                "selected_pair_mean_abs_error": float(selected_pair_metrics["mean_abs_error"]),
                "selected_pair_rmse": float(selected_pair_metrics["rmse"]),
                "pair_count": len(pairwise_values),
                "sample_count_per_pair": int(selected_pair_metrics["sample_count"]),
            }
        )

    with _estimator_benchmark_csv_path().open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=[
                "neighbor_estimator",
                ...
                "aggregate_rmse",
                "selected_pair_source_sensor",
                "selected_pair_target_sensor",
                "selected_pair_mean_abs_error",
                "selected_pair_rmse",
                "pair_count",
                "sample_count_per_pair",
            ],
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    _json_write(
        _estimator_benchmark_json_path(),
        {
            "example_k": EXAMPLE_K,
            "comparison_band_ids": list(COMPARABLE_PAIRWISE_BANDS),
            "selected_pair": {
                "source_sensor": HOLDOUT_BATCH_SOURCE_SENSOR,
                "target_sensor": HOLDOUT_BATCH_TARGET_SENSOR,
            },
            "estimators": comparison_rows,
        },
    )
...
def _plot_estimator_comparison(comparison_rows: list[dict[str, object]]) -> None:
    estimators = [str(row["neighbor_estimator"]) for row in comparison_rows]
    aggregate_mae = [float(row["aggregate_mean_abs_error"]) for row in comparison_rows]
    selected_pair_mae = [float(row["selected_pair_mean_abs_error"]) for row in comparison_rows]
    x = np.arange(len(estimators))
    width = 0.36

    figure, axis = plt.subplots(figsize=(8, 5.2))
    axis.bar(x - width / 2, aggregate_mae, width=width, color="#2a9d8f", label="All 12 source-target pairs")
    axis.bar(
        x + width / 2,
        selected_pair_mae,
        width=width,
        color="#e76f51",
        label=f"{SENSOR_BY_ID[HOLDOUT_BATCH_SOURCE_SENSOR].short_label} -> {SENSOR_BY_ID[HOLDOUT_BATCH_TARGET_SENSOR].short_label}",
    )
    axis.set_xticks(x, [estimator.replace("_", "\n") for estimator in estimators])
    axis.set_ylabel("Mean absolute error")
    axis.set_title("Held-out estimator comparison on the full-library example set")
    axis.grid(axis="y", alpha=0.25)
    axis.legend(frameon=False)
    figure.tight_layout()
    figure.savefig(DOCS_ASSETS_ROOT / "official_estimator_comparison.png", dpi=180)
    plt.close(figure)
...
def _sorted_segment_neighbors(segment_payload: Mapping[str, object], *, top_n: int) -> list[dict[str, object]]:
        ...
        rows.append(
            {
                ...
                "distance": neighbor_distances[index] if index < len(neighbor_distances) else float("nan"),
                "weight": neighbor_weights[index] if index < len(neighbor_weights) else 0.0,
                "band_values": neighbor_band_values[index] if index < len(neighbor_band_values) else [],
                "rank": index + 1,
            }
        )
    rows.sort(key=lambda row: (-float(row["weight"]), float(row["distance"]), int(row["rank"])))
    return rows[:top_n]
...
def _top_neighbor_row_ids(diagnostics_payload: dict[str, object]) -> set[str]:
    ...
    for result in diagnostics_payload["results"]:  # type: ignore[index]
        ...
        for segment_name in ("vnir", "swir"):
            ...
            for row in _sorted_segment_neighbors(segment_payload, top_n=OVERLAY_TOP_NEIGHBORS):
                row_ids.add(str(row["neighbor_id"]))
    return row_ids


def _render_holdout_neighbor_review_table(diagnostics_payload: dict[str, object]) -> list[str]:
    lines = [
        "| Sample | Segment | Rank | Neighbor | Distance | Weight | Source-fit RMSE | Query values | Neighbor source-band values |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for result in diagnostics_payload["results"]:  # type: ignore[index]
        sample_id = str(result["sample_id"])  # type: ignore[index]
        segments = result["diagnostics"]["segments"]  # type: ignore[index]
        for segment_name in ("vnir", "swir"):
            segment_payload = segments[segment_name]  # type: ignore[index]
            query_band_ids = [str(value) for value in segment_payload.get("query_band_ids", [])]  # type: ignore[arg-type]
            query_band_values = list(segment_payload.get("query_band_values", []))  # type: ignore[arg-type]
            query_summary = ", ".join(
                f"{band_id}={float(value):.4f}"
                for band_id, value in zip(query_band_ids, query_band_values)
                if value is not None
            )
            source_fit_rmse = segment_payload.get("source_fit_rmse")
            for row in _sorted_segment_neighbors(segment_payload, top_n=DOC_NEIGHBOR_TABLE_TOP_N):
                neighbor_values = ", ".join(
                    f"{band_id}={float(value):.4f}"
                    for band_id, value in zip(query_band_ids, row["band_values"])  # type: ignore[arg-type]
                )
                lines.append(
                    f"| `{sample_id}` | `{segment_name}` | `{row['rank']}` | "
                    f"`{str(row['neighbor_id']).split(':', 2)[-1]}` | "
                    f"`{float(row['distance']):.4f}` | `{float(row['weight']):.4f}` | "
                    f"`{float(source_fit_rmse):.4f}` | `{query_summary}` | `{neighbor_values}` |"
                )
    return lines
...
def _plot_holdout_neighbor_overlays(
    ...
    for row_axes, target in zip(np.atleast_2d(axes), HELD_OUT_TARGETS):
        ...
        for axis, segment in zip(row_axes, ("vnir", "swir")):
            ...
            for overlay_index, row in enumerate(top_neighbors, start=1):
                neighbor_id = str(row["neighbor_id"])
                distance = float(row["distance"])
                weight = float(row["weight"])
                neighbor_spectrum = neighbor_spectra[neighbor_id]
                color = neighbor_colors[(overlay_index - 1) % len(neighbor_colors)]
                axis.plot(
                    CANONICAL_WAVELENGTHS[wavelength_mask],
                    ...
                    linewidth=1.2,
                    alpha=0.9,
                    label=f"#{overlay_index} {neighbor_id.split(':', 2)[-1]} (d={distance:.3f}, w={weight:.3f})",
                )
                axis.scatter(
                    band_centers,
                    [float(value) for value in row["band_values"]],  # type: ignore[arg-type]
                    color=color,
                    s=16,
                    zorder=4,
                )
...
All generated outputs use `k = {EXAMPLE_K}` with
`neighbor_estimator = {EXAMPLE_NEIGHBOR_ESTIMATOR}`.
...
def _render_official_sensor_examples_doc(
    *,
    ...
    estimator_comparison_rows = _load_estimator_comparison_rows()
    neighbor_review_lines = _render_holdout_neighbor_review_table(_load_batch_diagnostics_payload())
    ...
    best_estimator = min(estimator_comparison_rows, key=lambda row: float(row["aggregate_mean_abs_error"]))
    best_selected_pair_estimator = min(
        estimator_comparison_rows,
        key=lambda row: float(row["selected_pair_mean_abs_error"]),
    )
    ...
    def _target_sensor_block(example: SelectedExampleRun) -> str:
        return _shell_block(
            ...
            f"  --neighbor-estimator {EXAMPLE_NEIGHBOR_ESTIMATOR}",
    ...
    batch_block = _shell_block(
        ...
        f"  --neighbor-review-output build/official_mapping_runtime/{_holdout_batch_neighbor_review_path().name}",
    )
...
<div class="fact-grid">
  ...
  <div><strong>Example estimator</strong><span>`{EXAMPLE_NEIGHBOR_ESTIMATOR}`</span></div>
...
All generated outputs on this page use `k = {EXAMPLE_K}` with
`--neighbor-estimator {EXAMPLE_NEIGHBOR_ESTIMATOR}`.
...
- pairwise metrics on this page were generated with `neighbor_estimator = {EXAMPLE_NEIGHBOR_ESTIMATOR}`
...
- best aggregate held-out mean absolute error:
  `{float(best_estimator["aggregate_mean_abs_error"]):.4f}` with `{best_estimator["neighbor_estimator"]}`
- best {SENSOR_BY_ID[HOLDOUT_BATCH_SOURCE_SENSOR].short_label} ->
  {SENSOR_BY_ID[HOLDOUT_BATCH_TARGET_SENSOR].short_label} held-out mean absolute error:
  `{float(best_selected_pair_estimator["selected_pair_mean_abs_error"]):.4f}` with
  `{best_selected_pair_estimator["neighbor_estimator"]}`
...
- the top `{OVERLAY_TOP_NEIGHBORS}` weighted neighbors from the segment’s
  `k = {EXAMPLE_K}` shortlist on the full library, labeled with source-space
...
{chr(10).join(neighbor_review_lines)}
...
[`examples/official_mapping/results/selected/{_holdout_batch_neighbor_review_path().name}`](../examples/official_mapping/results/selected/{_holdout_batch_neighbor_review_path().name})
...
def _load_estimator_comparison_rows() -> list[dict[str, str]]:
    with _estimator_benchmark_csv_path().open("r", encoding="utf-8", newline="") as handle:
        return list(csv.DictReader(handle))
...
def build_official_mapping_examples(*, siac_root: Path | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="official_mapping_runtime_", dir=str(TMP_ROOT)) as tmpdir:
        ...
        estimator_comparison = _write_neighbor_estimator_holdout_comparison(prepared_root, truth_rows_by_sensor)
        metrics = _write_pairwise_metrics(
            prepared_root,
            truth_rows_by_sensor,
            neighbor_estimator=EXAMPLE_NEIGHBOR_ESTIMATOR,
        )
    ...
    _plot_estimator_comparison(estimator_comparison)
    _plot_holdout_batch_examples(sensor_payloads, truth_rows_by_sensor)
    _plot_holdout_neighbor_overlays(truth_spectra, neighbor_spectra, sensor_payloads)
